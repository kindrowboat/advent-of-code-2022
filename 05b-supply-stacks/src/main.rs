...
impl Stacks {
    ...
    pub fn process_instruction(&mut self, instruction : &str) {
        ...
        let from_stack_number : usize = caps.get(2).map_or("", |m| m.as_str()).parse().unwrap();
        let to_stack_number : usize = caps.get(3).map_or("", |m| m.as_str()).parse().unwrap();
        let from_stack = self.map.get_mut(&from_stack_number).unwrap();
        let move_start_index = from_stack.len() - count;
        let mut moving : Stack = from_stack.drain(move_start_index..).collect();
        let to_stack = self.map.get_mut(&to_stack_number).unwrap();
