...
fn is_complete_overlap(line: &str) -> bool {
    let range_pair = parse_ranges(line);
    (range_pair.0.0 >= range_pair.1.0 && range_pair.0.1 <= range_pair.1.1 ) ||
    (range_pair.1.0 >= range_pair.0.0 && range_pair.1.1 <= range_pair.0.1 )
}
